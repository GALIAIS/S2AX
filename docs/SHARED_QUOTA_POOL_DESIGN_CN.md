...
V1 的目标是建立一个以**真实已记录费用**为计量单位、以**共享池硬上限**为安全边界、以**权重公平份额**为默认分配、允许**受控借用**的控制平面。它不改变现有计费价格、不改变余额模式、不伪造上游供应商的额度数据。
...
上游真实额度若能由某个平台稳定获取，应通过独立的 quota adapter 将观测值写入 `upstream_*` 字段；没有观测值时必须使用管理员配置的保守美元等价上限。

## 3. 核心概念与不变量

### 3.1 计量口径

- 计费单位：美元等价的 `actual_cost`，与现有订阅扣费一致；
...
管理员为每个窗口配置 `capacity_usd` 为该上游窗口的保守美元等价容量。每个窗口独立计算：
...
默认值：reserve 15%、soft stop 85%、hard stop 95%。因此配置 $1,000 时，默认可分配 $850，硬停止线为 $807.50。剩余空间不是必须消耗的额度，而是供应商波动、迟到用量和并发超支的安全缓冲。
...
当前网关没有一个对所有平台都可靠的“官方美元额度”接口。V1 将 `capacity_usd` 作为明确的保守人工输入，并保留两个上游观测字段。后续 adapter 应满足：

1. 只写入观测值，不直接改变管理员容量；
2. 观测值有时间戳、来源和置信度；
3. 自动建议值必须经过上下界与 EMA 平滑；
4. 任何上游接口失败都不能把容量变成 0；
5. 管理员确认后才将建议值应用为新容量。

对 Pro 20x 拼车，5h 和 7d 必须分别估计容量、分别设置预留；推荐先按各自窗口的
“最近窗口实际使用 / 上游利用率”估计，再减去 15%～20% 预留，不能用 7d 容量替代 5h 容量。
