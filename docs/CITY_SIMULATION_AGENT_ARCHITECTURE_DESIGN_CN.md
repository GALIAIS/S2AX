...
状态：设计契约冻结；A0/A1/A2/A3.1/A3.2 已实现。A3.3 已完成 Case、social、程序性 receipt/归属/分派、结构化任务、有限导航计划与服务器拥有的单格交通容量预约。`1.13.0` 只新增 `character.navigation.plan`：Agent 只能从当前 Observation 中服务端公布的有限静态入口 Portal 候选回显一个目的地；服务端用固定、有界的搜索产生最多 32 步的临时计划，并在每个 movement quantum 重新验证和结算一格。新 world 还会以独立、不升级 Agent policy 的 traffic binding 在同一 movement frame 先封存单格 capacity receipt、后消费精确移动；该协议不向模型、用户或浏览器暴露路径、坐标、占用、容量或通行权。计划和 receipt 只存在于 owner-private append-only binding/head/event 链，不能改写奖励或资产。所有 slice 都接入版本化 policy、sealed action context、异步 decision/intent/due-event、数据库 hash chain 与 reducer 验证。Case 发起/裁决、复杂关系、外部模型路由、预算、NPC 微观生命周期与奖励仍未上线；当前 `autonomous` 仍不是可对外承诺的生产 AI 游玩能力。
...
当前 `city-openworld-v24` 已具备开放世界 Actor、属性、Role、Status、位置、Portal、导航、Rule/Case、设施、交通、货运、事实、快照和回放等底座；也已有平台虚拟货币钱包、封账账本、分组策略和签名集成入口。`city-openworld-realtime-v2` 现有十四代封存 policy：`1.0.0` 为 A0/A1 树和生命周期基线，`1.1.0` 为 A2 的 wait-only 决策链，`1.2.0` 为 A3.1 的用户角色自治活动闭环，`1.3.0` 为 A3.2 的有限移动/Portal/Role action adapter，`1.4.0` 为 A3.3 的既有 Law Case 确认回执，`1.5.0` 为 A3.3 的有界社交问候 adapter，`1.6.0` 为已确认 Case 的程序性复核回执，`1.7.0` 为无自由文本、无证据效力的相邻公开 Actor Case-report receipt，`1.8.0` 在不扩展 action allowlist 的前提下为该 receipt 建立独立、可过期的 `evidence_required` Case-intake work item，`1.9.0` 为既有、已封存的角色 Law event 建立独立的 `server.sealed_law_event` 证据来源句柄，`1.10.0` 再以唯一、active、未占用、同 subject 且先于 receipt 的 handle 建立可关闭的程序归属记录，`1.11.0` 只为已有 assignment link 创建有界的程序分派/关闭回执，`1.12.0` 新增有限 `character.task.accept` 与 action-context v5 的 `available_task_codes`，把任务完成严格绑定到后续的匹配 Agent activity fact，`1.13.0` 新增 `character.navigation.plan` 与 action-context v6 的 `available_navigation_destination_portal_codes`，把有限目的地选择严格绑定到服务端静态入口 Portal、有限搜索和逐 quantum 重验证的移动事实。新建 realtime world 绑定 `city-realtime-agent-core@1.13.0`；既有 `1.0.0–1.12.0` world 保持原 policy、行为、授权散列与 canonical hash，不被自动升级。
...
- `1.1.0` 仍严格只允许 `agent.wait {}`；`1.2.0` 仍只桥接受限活动；`1.3.0` 仍只桥接一格移动、现有 Portal 与现有 Role 变更；`1.4.0` 只能确认服务端已封存且属于本人的 Law Case，不能发起、改写、裁决或撤销 Case；`1.5.0` 只能对服务端公布的相邻 active NPC/Character 生成无自由文本的 greeting 事实，不能聊天、改变对方状态、发奖或改变关系以外的任何领域状态；`1.6.0` 只能在服务端公布的本人、已确认、未曾复核且仍位于 15 分钟 world-time 窗口内的 Case code 上提交一次 `character.case.review.file`，固定 30 秒后只能得到 `closed_no_change`；`1.7.0` 只能从同一 sealed Observation 的相邻公开 target catalogue 选择一次 `character.case.report.file`，生成 `filed_unverified` receipt；`1.8.0` 的模型 action surface 与 `1.7.0` 完全相同，但服务端会从同一 receipt 派生一条独立的 `evidence_required` work item，30 秒后只能变为 `expired_no_evidence`；`1.9.0` 的 action/Observation wire shape 仍与 `1.8.0` 完全相同，仅允许服务端在既有活动 reducer 已封存的 Law event 后创建一个确定性 `server.sealed_law_event` handle；`1.10.0` 继续保持该 wire shape 与 action allowlist，只会由 reducer 在 receipt 落库时关联唯一一个先前 active、同 subject、未占用的 handle，并会在来源失效时关闭关联；`1.11.0` 仍不增加 action 或 Observation 字段，只会由服务端在同一 report frame 为既有 assignment link 追加 `procedure_queued`，并在来源窗口关闭时固定为 `source_window_closed`。receipt、work item、handle、关联与分派都没有自由文本、理由、证据声明或源内容；分派不能验证 evidence、不能自动改变 intake，也不能进入裁决、处罚、城市信用、库存、账本、平台资产或奖励路径；
...
- `1.12.0` 只允许从当前 action context 的 `available_task_codes` 中选择一个已发布任务；接受后写入 owner-private task head/event，并固定预约一个 5 分钟 server-owned expiry due event。仅同一 autonomous Character Agent 在截止前完成任务指定的既有活动时，才会同 frame 追加 `task_completed`；手工活动、错配活动、deadline 时刻活动、浏览器请求和模型文本均不能完成任务。`GET /api/v1/city/worlds/:world_id/realtime/character/tasks` 只返回当前 owner 的 run code、任务/活动 code、状态、revision 与 frame/deadline，不返回 intent、Agent、activity hash、人格、模型、奖励或资产字段。
- `1.13.0` 只允许从当前 action context 的 `available_navigation_destination_portal_codes` 回显一个服务端公布的静态 `entrance` Portal code。服务端仅从当前地表、静态、可通行的入口端点中确定性挑选最多 8 个候选，并以固定邻居顺序在最多 32 步、4096 个展开节点内计算路线；模型从不接收或提交路径、坐标、层级、占用、路线缓存或预约。每个 `system.realtime.character_navigation_step` 只消费一格，并在执行时重新校验 Actor 位置、计划 revision、静态地形、动态占用和控制状态；变化只会封存 `blocked` 或 `cancelled`，不改写已选目标。`GET /api/v1/city/worlds/:world_id/realtime/character/navigation-plans` 只返回当前 owner 的 run code、destination portal、状态、revision、step 数和 frame，不返回 intent、Agent、完整路径、其他角色位置、人格、模型、奖励或资产字段。
- A3.3d 在不改变 `1.13.0` action allowlist 或 Observation wire shape 的前提下，为新 world 绑定 immutable `city-realtime-pedestrian-capacity@1.0.0`。每个 active plan 的下一 movement boundary 都由 priority 70 的 `system.realtime.character_traffic_reservation` 先重算一格、按 canonical due order 判定 `granted|denied_capacity`；priority 90 的导航 reducer 只能消费同 revision、同 from/to、同 frame 的 grant 并写入 position hash。owner API 只返回自己的粗粒度 receipt；没有坐标、slot、他人、路径、Agent、intent、provider、资产或写入端点。没有 binding 的历史 1.13 world 保留原导航语义，不被升级。
- Rule/Case 发起与裁决、复杂关系、经济和跨计划的交通网络仍尚未作为 Agent action adapter 或系统协议落地；不要把当前受限活动/空间/职业/Case acknowledgement/social greeting/结构化任务/有限导航与单格容量支持宣传为完整自动生活系统。
...
### 7.2.1 已实现的 A2 / A3.1 / A3.2 / A3.3 受限闭环、Case-process、结构化任务与有限导航计划

`city-realtime-agent-core@1.1.0` 保持 A2 的 wait-only 闭环；`1.2.0` 在其上增加用户 Character Agent 的最小自主活动闭环；`1.3.0` 增加严格有限的空间/职业 adapter；`1.4.0`、`1.5.0`、`1.6.0` 和 `1.7.0` 增加四个互相独立的、版本化的 A3.3 action adapter；`1.8.0` 保留 `1.7.0` 的 action 与 Observation wire shape，仅新增服务器侧 Case-intake work item；`1.9.0` 继续保持该 wire shape 与 action allowlist，仅增加从既有 sealed Law event 衍生、由服务端独占创建的短时 evidence-source handle；`1.10.0` 仍不增加 action 或 Observation 字段，只增加唯一、可关闭的 server-side source correlation 与 owner-safe process projection；`1.11.0` 继续保持全部模型契约，只为已有 assignment link 增加 server-owned 的有限程序分派状态；`1.12.0` 以 action-context v5 新增有限 `character.task.accept`，并把 owner-private task 的完成严格绑定到后续同一 autonomous Agent 的匹配活动事实；`1.13.0` 以 action-context v6 新增有限 `character.navigation.plan`，把 destination portal choice 严格绑定到当前 Observation 的服务端静态入口候选，并把路线逐格执行绑定到 server-owned plan/due-event 链；A3.3d 以独立 immutable capacity policy/world binding 为该已有路线补上 server-only 单格预约，不改变模型 action、policy version 或 Observation。所有版本都没有放宽执行边界：
...
- worker 在事务外运行；`1.1.0` 的 fake provider 只能输出 `agent.wait {}`，`1.2.0` 的 fake provider 只可在服务端公布的当前可用活动中选择 `character.activity.perform`，`1.3.0` 的管理/测试选择器只能从同一 Observation 的有限 `action_context` 中选择活动、移动、Portal 或 Role，`1.4.0/1.5.0/1.6.0/1.7.0/1.8.0/1.9.0/1.10.0/1.11.0` 只能选择已公布的 Case/social/Case-review 或同一 public-adjacency catalogue 中的 Case-report target，`1.12.0` 只能从同次 Observation 的 `available_task_codes` 回显一个任务 code，`1.13.0` 只能从同次 Observation 的 `available_navigation_destination_portal_codes` 回显一个静态入口 Portal code。`1.8.0/1.9.0/1.10.0/1.11.0` 都不向模型公布 evidence、受理队列、证据句柄、关联、程序分派或裁决能力；`1.9.0` 的句柄只能由活动 reducer 在同一 sealed Law event 已落库后捕获，`1.10.0` 的关联只能由报告 due-event reducer 在同帧计算，`1.11.0` 的分派只能消费同帧产生的 assignment link，不能由报告参数、模型、提示词、浏览器请求或手工 API 选择来源。`1.12.0` 的任务截止时间、过期 due event、完成 activity hash 与状态转移，以及 `1.13.0` 的 destination、路线候选、plan revision、下一步 due event 和每一格位置事实都由服务端封存；模型与浏览器都不能自行完成、续期、指定路径、占用格或改写计划。所有版本都经过 lease、attempt、hash、envelope、decision、intent 与 due-event reducer 的完整链路。
- intent 的最早执行时间为下一 world-time quantum，且在 due-event reducer 再次验证 Agent control/precondition；`character.activity.perform` 复用既有活动/生活/进展/库存/规则 reducer，`character.move` 复用相邻、地形/室内与占用检查及 Actor position event，`character.portal.traverse` 复用 Portal 拓扑/占用检查，`character.role.change` 复用资格、assignment、progression event 与 Profile reducer，`character.navigation.plan` 只创建有限的 owner-private 计划并由后续 server-owned due event 逐格复用既有 position event。过期、状态变化或异常 payload 只变为 stale/rejected/blocked/cancelled，不能修改 Actor、账本、Rule 或奖励。
...
该阶段证明了人格私有性、控制撤销、时间隔离、有限行动上下文、受控活动/空间/职业执行、既有 Case 回执、无文本社交事实、无裁决权的程序性 Case-review 回执、无证据效力的 Case-report receipt、从 receipt 隔离出的 `evidence_required → expired_no_evidence` work item、独立且可失效的 sealed-Law evidence-source handle、唯一 source correlation、owner-safe process projection、受限程序分派、可解释关系投影、不带奖励的结构化任务、有限逐格导航以及同帧单格容量 receipt 可以共同回放；它不代表角色已能自主规划生活、发起或裁决法律程序、使用真实大模型或获得平台奖励。关系说明是基于已封存 social head/event 的只读重算，使用封闭的 contact tier、方向模式、最后互动方向和 reason code；它不增加关系权利、聊天、私有记忆、法律状态、奖励或任何资产变化。`server.sealed_law_event` 的存续、`linked_active/source_window_closed` 关联和 `queued/source_window_closed` 分派只表示“一个原有 Law event 在规定窗口内被一个程序 receipt 引用、已经入队或其窗口已关闭”，不表示事件证明了任何 report，也不会产生 Case、指控、处罚、奖励或任何资产变化。结构化任务只表示已封存的 Agent 工作目标在固定时效内被匹配活动完成或过期，也不产生奖励、钱包或资产变化；导航计划与容量 receipt 也只表示服务端逐格执行有限目标时的即时共享 cell 仲裁，不能预占未来路线、制造长期通行权或跳过动态占用。下一步按依赖进入外部 provider/预算的 A4；每个 slice 仍必须保持独立版本、有限状态机、权限边界、时效、回放与撤销语义，继续禁止把 `filed_unverified` 或程序状态自动提升为法律结论。
...
#### A3.3 第一至第四个 action slice、Case-process、独立证据 source 与受限归属实现记录（2026-07-22）
...
- `relations` 同时返回版本化 `explanation`：由 relation head 与同一 append-only event chain 只读重算 `contact_tier`、`interaction_pattern`、`last_interaction_direction` 与固定 `reason_codes`。方向只指向调用者自己的已参与 greeting，不能枚举他人关系或读取聊天、人格、Agent、模型、provider、提示词、账户、钱包、奖励或法律数据。该 projection 不引入 policy/action/context/migration，也不写 canonical state；历史 social world 的既有 event/hash chain 保持不变。
...
- 迁移 `274_city_realtime_agent_case_report_intake.sql` 发布 `city-realtime-agent-core@1.7.0` 的 `character.case.report.file`。它**不引入** action-context v5：复用 v4 已封存的 `available_social_target_codes`，因此候选仍是服务端筛出的同层相邻 active NPC/Character；finalizer 与 due-event reducer 均再验证 target、生命周期、位置、policy binding 和 source intent。
- Case-report head/event 只允许 ordered `(reporter_actor_code, subject_actor_code)` 一次 `filed_unverified` 写入。其 binding、genesis/event/state hash 与 PostgreSQL guard 同时约束 actor kind、active user Character Agent、相邻 public target、pending exact intent 和 immutable head；没有文本、理由、证据、Case code、Rule、罚款或奖励字段。owner-scoped service projection 只返回自己报告过的 public target 摘要、status 和 frame，不返回 Agent/intent/hash/人格/模型/provider/平台资产。
- 迁移 `275_city_realtime_agent_case_intake_work_items.sql` 发布 `city-realtime-agent-core@1.8.0`。它不增加 action、不引入 action-context v5，也不把 report 解释为证据；在 `filed_unverified` receipt 已通过 1.7 的 sealed target/intent 检查后，服务器才同帧创建与 report event hash 绑定的 `evidence_required` head/event，并预写唯一的 `system.realtime.character_case_intake_expire` due event。30 秒到期时只能追加 `expired_no_evidence`，不得触碰 Law Case、Rule、city credit、库存、账本、平台/虚拟货币或奖励。
- intake binding/head/event 与过期 due event 都有独立 binding/genesis/state/event hash、exact payload/aggregate/dedup、policy gate 和 PostgreSQL guard；head 只能从 `evidence_required` 变为 `expired_no_evidence`。当前没有 member-facing intake API，避免向普通玩家暗示未验证 report 具有案件效力；canonical state/replay 仍完整保留其程序事实。
- 迁移 `276_city_realtime_agent_case_evidence_sources.sql` 发布 `city-realtime-agent-core@1.9.0`。它不增加 action、不引入 action-context v5，也不从 report/intake 派生证据。只有既有角色活动 rule reducer 已先行封存、且可重新校验 hash 的 Law event，才能在同一 sealed frame 生成确定性 `evidence.law.<law_event_hash>` source handle；handle 只保存 source event 的 hash/sequence、subject、frame 与生命周期，不复制 Rule 文本、Case 内容、模型输出、提示词、报告内容、账户或资产数据。
- 该 handle 的 genesis/head/event hash chain、exact source FK、固定 source kind、固定 30 秒 `system.realtime.character_case_evidence_expire` due event 与 mutation gate 由 PostgreSQL 和 reducer 双重约束。它只可从 `active` 变为 `expired`；失效不会删除、改写或降级原 Law event，也不会修改 report、intake、Law Case、Rule、city credit、库存、账本、平台/虚拟货币、处罚或奖励。当前没有 member-facing evidence API，也没有 assignment/case/adjudication 路径。
- 迁移 `277_city_realtime_agent_case_evidence_assignments.sql` 发布 `city-realtime-agent-core@1.10.0`。它不增加 action、不改变 action-context v4 或 Observation wire shape。报告 reducer 只在同 subject 下找到**恰好一个**尚未占用、`active`、来源/capture frame 早于 receipt 且 expiration 晚于当前 world-time 的 `server.sealed_law_event` handle 时，才同帧创建 `linked_active` assignment；零个或多个候选都不关联，任何来源最多关联一个 report。handle 到期时，既有 evidence reducer 只追加 `source_window_closed` event 并关闭 assignment，不回写 report/intake 或任何 Law/经济状态。
- assignment binding/head/event 使用独立 genesis/state/event hash chain、exact report/intake/evidence/Law/source-frame FK、单来源唯一约束和 reducer-only PostgreSQL guards。owner-scoped `GET /api/v1/city/worlds/:world_id/realtime/character/case-process` 仅返回该用户角色的公开 subject 摘要、`filed_unverified`、粗粒度 intake status、`not_matched/linked_active/source_window_closed` 和 frame；不返回 evidence code/hash、Law rule/case/处罚、报告内容、其他 owner、Agent、模型、prompt、账户、钱包或奖励。
- 迁移 `278_city_realtime_agent_case_procedure_dispatches.sql` 发布 `city-realtime-agent-core@1.11.0`。它不增加 action、不改变 action-context v4 或 Observation wire shape。只有同一 report frame 已创建 `linked_active` assignment 时，服务器才创建一个关联 assignment 第一个 link event hash 的 `procedure_queued` receipt；source expiry 会在同一 due-event reducer 内先关闭 assignment，再关闭该分派为 `source_window_closed`。分派 binding/head/event 只有 `queued/source_window_closed` 两态，独立封存 genesis/state/event chain，并通过 exact assignment-event FK、link hash、frame 与 reducer-only PostgreSQL guard 约束。
- `case-process` owner projection 只新增 `procedure_dispatch_status` 的 `not_routed/queued/source_window_closed` 粗粒度状态；不返回 assignment link hash、evidence code/hash、Law rule/case/处罚、报告内容、其他 owner、Agent、模型、prompt、账户、钱包或奖励。真实 PostgreSQL 已覆盖 `sealed Law source → unique report correlation → queued dispatch → source_window_closed` 全链路；另有 reducer/hash/arguments/context/policy compatibility/迁移 strictness 覆盖。后续关系或任务切片不能从 `filed_unverified`、模型输出或浏览器请求直接派生法律结论或处罚。
- 迁移 `279_city_realtime_agent_character_structured_tasks.sql` 发布 `city-realtime-agent-core@1.12.0`、action-context v5 及唯一的新 action `character.task.accept`。v5 在保持 v1–v4 的既有字段和排序语义上，显式加入 `available_task_codes`；旧 policy 不会收到该字段或新 action。当前 catalog 固定 `task.civic.cleanup → civic.cleanup` 与 `task.civic.shift → work.civic_shift` 两个内容定义任务，任何未来扩展都必须发布新的 catalog/policy/migration，不可借助任意字符串放宽。
- 接受任务要求同一 pending sealed intent、当前可用候选、无已接受任务，并预先写入唯一 `system.realtime.character_task_expire` due event；task head/event 以独立 binding、genesis/state/event hash chain 封存。任务没有专属 reward：只有原有活动 reducer 可更新角色生活/进展状态。活动完成仅当 autonomous Agent 的同一 actor、同一 task activity、早于 deadline 的已封存 activity event 精确匹配时发生；到期只能追加 `task_expired`。SQL gate 同时拒绝直接 head/event 改写、伪造 deadline、伪造活动 hash 或跨 actor/source intent 关联。
- owner-scoped `GET /api/v1/city/worlds/:world_id/realtime/character/tasks` 没有 mutation 对应接口，仅读取调用者自己的最小投影。真实 PostgreSQL 回归已覆盖 `wakeup → task decision → pending intent → accepted task/deadline → matching activity → completed task`、owner 隔离和 direct SQL 拒绝；纯 reducer/hash 测试同时覆盖错配 activity、deadline 边界和 expiry transition。
- 迁移 `280_city_realtime_agent_character_navigation_plans.sql` 发布 `city-realtime-agent-core@1.13.0`、action-context v6 及唯一的新 action `character.navigation.plan`。v6 在保持 v1–v5 的既有字段和排序语义上，显式加入 `available_navigation_destination_portal_codes`；候选是服务端从当前 static、surface、`entrance` Portal 端点确定性筛出的有限集合，而不是任意 destination、路径或坐标。
- 计划只允许同一 active autonomous Character Agent 的 pending sealed intent 创建。服务端以固定邻居顺序在当前静态地表上做有界搜索（最多 8 个目的地候选、32 步、4096 个展开节点），将 `planned → step* → arrived|blocked|cancelled` 封存在独立 binding/head/event hash chain，并预写唯一 `system.realtime.character_navigation_step` due event。下一步到期时会从持久化 Actor 位置重算一格，重新检查 plan revision、控制模式、地形与动态占用；成功仅复用既有 Actor position event，失败只封存固定 reason，不会复写 destination 或偷换路线。
- owner-scoped `GET /api/v1/city/worlds/:world_id/realtime/character/navigation-plans` 只读取调用者自己的 run code、destination portal、status、revision、step 数和 frame；没有计划创建、修改、取消或完整路径读取 API。控制撤销会在同一事务取消 active plan 和未来 navigation due event。真实 PostgreSQL 回归已覆盖 `wakeup → navigation decision → pending intent → planned head/event → exact next position event`、candidate 约束、owner 隔离、控制撤销与 direct SQL 拒绝；纯 reducer/hash 测试覆盖步骤、到达、阻塞与取消。
...
| A3.3b | Case 发起/流程、丰富关系、结构化任务与行为 overlay action adapter | A3.3a | 已完成 Case-process、关系投影和首个结构化任务；每个后续关系/行为 slice 仍需独立 scope/schema/执行时重检/回放与撤销语义，不能将内容、法律结论或结果交给模型生成 |
| A3.3c / A3.3d | 有限导航计划与实时单格容量预约 | A3.3b | 已完成：1.13 的服务端有限 destination/逐格导航，以及 immutable capacity binding、traffic-before-navigation due order、同帧 `granted → consumed` position receipt。下一步是 A4；未来 cell/edge 多容量、车辆或长期/可释放协议必须另行版本化，不能将路径、坐标、占用或通行权交给模型或浏览器 |
...
15. `1.12.0` 上 wakeup 产生有限任务候选后，Agent 接受一个任务必须同时写入 sealed intent、accepted head/event 与唯一 expiry due event；只有同 actor、同 activity code、早于 deadline 的后续 autonomous activity 能完成任务。错配 activity、手工/浏览器写入、到期边界和 direct SQL 都不得完成或改写；owner 只能读取自己的最小任务投影。
16. `1.13.0` 上 wakeup 产生有限入口候选后，Agent 选择一个 destination portal 必须同时写入 sealed intent、planned head/event 和唯一下一步 due event；每次 due event 只能从当前 Actor 位置推进一格，并精确链接位置事实。占用、控制撤销、计划 revision 或静态通行变化只能封存 blocked/cancelled；模型、浏览器、其他 owner 与 direct SQL 都不得指定/读取完整路径或改写计划。
17. 绑定 A3.3d capacity runtime 的新 world 中，每个 active plan 的下一次 movement boundary 必须同时存在 traffic due event；它以 priority 70 在同一 frame 产生唯一 `granted|denied_capacity` receipt，priority 90 导航只有消费同 revision、同 from/to、同 position hash 的 grant 才能移动。旧 1.13 world 未绑定时保持原语义；owner 只读最小 receipt，outsider/direct SQL 均不能越权。
