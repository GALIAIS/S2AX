...
版本：v1.6（2026-07-19）
状态：F0–F8.2 基础、开放世界空间、持续移动、设施生命周期与物理网络闭环已实现；当前引擎为 `city-f8-v3`，下一入口为 F8.3/F8.4
关联文档：《城市模拟游戏总体设计》《城市模拟 F6.2 人口迁移设计与验收》《城市模拟 F6.3 家庭生命周期与收入层迁移详细设计》《城市模拟 F7.0 空间规则与坐标底座详细设计》《城市模拟 F7.7 Actor 空间与控制权详细设计》《城市模拟 F7.8 成员、调度与命令回执详细设计》《城市模拟 F7.9 Actor 通行、占用与确定性导航详细设计》《城市模拟 F7.10 动态 Portal 与访问控制详细设计》《城市模拟 F7.11 NPC 行动预算、确定性重规划与拥堵控制详细设计》《城市模拟 F8.0 通用公共设施与城市服务底座详细设计》《可扩展虚拟货币与权益体系设计》

> 本文是已实现基础内核的实施索引。总体边界、阶段顺序和新 worldgen 策略以《城市模拟游戏总体设计》v1.0 为准；本文不再定义后续阶段的产品优先级。
...
- `city-f7-v2`–`city-f7-v6` 地块/建筑、开发施工、企业经营场所、开放世界 Actor、属性/Role/Rule/Fact/Effect、权威位置、控制 grant 与完整重放恢复。
- active 城市成员目录、owner 成员管理、按用户身份选择控制委托，以及 owner/本人隔离的稳定命令回执 API。
- 数据库持久 Tick lease、崩溃接管、指数退避、确定性 scheduler idempotency key、运行速度调度和应用关闭清理。
- `city-f7-v7` Terrain/Furniture/Building/Portal/Actor Occupancy 统一通行解析、repeatable-read 确定性有界路径查询、Tick 内移动重校验和前端逐步导航。
- `city-f7-v8` 每 Portal 唯一版本化状态、open/closed/locked 状态机、声明式 Requirement 授权、路径与移动双重阻断、Fact/Effect、重放恢复和结构化前端策略工作台。
- `city-f7-v9` 通用 movement intent、整数行动预算、稳定公平调度、逐 Tick 重规划、Cell/Edge reservation、终态原因、Fact/Effect、重放恢复和局部刷新前端工作台。
...
- 奖励 Outbox、完整可玩产品层、NPC 需求/任务效用 AI 和长期离线追赶调度；显式、幂等的单步执行 API 与 F7.8 自动调度并存。
...
F1 首次使用 `city-f1-v1` 版本；F2、F3、F4、F5、F6.1–F6.3、F7.1–F7.11 依次升级状态契约，当前执行版本为 `city-f7-v9`。旧版本继续作为可运行、可回放的兼容版本保留，升级只能沿显式单步路径进行。默认模拟时间从 `2000-01-01T00:00:00Z` 开始，每个 tick 固定推进一小时；数据库当前时间只记录提交和耗时，不进入规则或状态哈希。
...
16. **已完成：F7.7 `city-f7-v6` Actor 权威位置、移动、控制 grant、空间作用域规则、地图 glyph 与重放恢复。**
17. **已完成：F7.8 城市成员目录、用户选择器、持久 Tick lease/失败退避、私有命令回执与无整页刷新前端闭环。**
18. **已完成：F7.9 `city-f7-v7` 静态通行、Actor 占用、Portal/楼梯、确定性路径、移动重校验和路径工作台。**
19. **已完成：F7.10 `city-f7-v8` 动态 Portal 状态、声明式访问授权、导航阻断、Fact/Effect、规范状态、重放恢复和策略工作台。**
20. **已完成：F7.11 `city-f7-v9` movement intent、行动预算、确定性重规划、稳定公平调度、拥堵预留、重放恢复和移动工作台。**
21. **已完成：F8.0 `city-f8-v1` 版本化设施/服务目录、真实空间设施、容量与主体需求、连接、确定性分配、不可变结算、规范状态、重放恢复、查询 API 和操作工作台。**
22. **已完成：F8.1 `city-f8-v2` 设施投运/维护/维修/退役、人员覆盖、磨损故障、材料/总账/预算守恒、规范状态、重放恢复和查询 API。**
23. **已完成：F8.2 `city-f8-v3` 通用物理网络图、整数容量路由、逐段损耗、流量守恒、故障隔离、规范状态、重放恢复、诊断 API 和拓扑工作台。**
24. 当前按要求停止；后续恢复时从 F8.3 社会服务可达性开始，F8.4 跨域效果以及 E1 银行信贷、E2 证券继续保持独立可选分支。
