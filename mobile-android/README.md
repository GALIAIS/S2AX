...
- 工作台：按权限汇总网页端的服务、资产、城市、运营、资源、计费和治理数据；支持模块搜索、分页、筛选、核心指标卡、趋势折线、模型/分组分布和逐层数据详情预览。字段会按移动端语义显示时间、毫秒与字节单位，系统返回的模块路径固定在应用内，不允许输入任意管理接口。
- 城市：可选择共享世界，并按世界总览、地图与空间、服务基础设施、交通经济、角色规则、历史审计和管理员运行控制分组查看实时数据；系统返回键会逐层回退，不会直接退出工作台。
