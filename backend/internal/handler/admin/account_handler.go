...
func (h *AccountHandler) Create(c *gin.Context) {
	...
	if err := service.ValidateOpenAIImageGenerationExtra(req.Platform, req.Extra); err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
type TestAccountRequest struct {
	...
	// OpenAIImageTextModel 是 OAuth 生图桥接的 Responses 外层文本模型覆盖值。
	OpenAIImageTextModel string `json:"openai_image_text_model"`
...
func (h *AccountHandler) Test(c *gin.Context) {
	...
	opts := service.AccountTestOptions{
		...
		OpenAIImageTextModelID: req.OpenAIImageTextModel,
	}
...
func (h *AccountHandler) ApplyOAuthCredentials(c *gin.Context) {
	...
	if err := service.ValidateOpenAIImageGenerationExtra(existing.Platform, req.Extra); err != nil {
		response.ErrorFrom(c, err)
		return
	}
	if err := service.ValidateUpstreamRequestIDHeaderExtra(req.Extra); err != nil {
...
func (h *AccountHandler) BatchCreate(c *gin.Context) {
	...
	for _, item := range req.Accounts {
		...
		if err := service.ValidateOpenAIImageGenerationExtra(item.Platform, item.Extra); err != nil {
			response.ErrorFrom(c, err)
			return
		}
	}
...
func (h *AccountHandler) GetAvailableModels(c *gin.Context) {
	...
	if account.IsOpenAI() {
		...
		if account.IsOpenAIPassthroughEnabled() {
			response.Success(c, service.AppendOpenAIImagePickerModels(account, openai.DefaultModels))
			return
		}
		...
		response.Success(c, service.AppendOpenAIImagePickerModels(account, models))
		return
	}
