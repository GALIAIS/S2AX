...
func TestParseCodexSessionImportEntriesUnwrapsSub2APIExport(t *testing.T) {
	...
	req := CodexSessionImportRequest{
		Content: fmt.Sprintf(`{"exported_at":"2026-07-16T00:00:00Z","proxies":[],"accounts":[{"name":"Exported account","platform":"openai","type":"oauth","expires_at":%d,"auto_pause_on_expired":true,"credentials":{"access_token":%q,"chatgpt_account_id":"acct-export","chatgpt_user_id":"user-export","email":"export@example.com","plan_type":"plus"},"extra":{"name":"Exported account","openai_oauth_responses_websockets_v2_enabled":true,"openai_oauth_responses_websockets_v2_mode":"passthrough","openai_ws_force_http":false,"openai_compact_mode":"force_on","openai_long_context_billing_enabled":true,"codex_cli_only":true,"codex_fingerprint_mode":"session","codex_fingerprint_seed":"22222222-2222-4222-8222-222222222222","access_token":"must-not-copy"}}]}`,
	...
	if item.Extra["openai_oauth_responses_websockets_v2_enabled"] != true {
		t.Fatalf("ws enabled = %v, want true", item.Extra["openai_oauth_responses_websockets_v2_enabled"])
	}
	if item.Extra["openai_oauth_responses_websockets_v2_mode"] != "passthrough" {
		t.Fatalf("ws mode = %v, want passthrough", item.Extra["openai_oauth_responses_websockets_v2_mode"])
	}
	if item.Extra["openai_ws_force_http"] != false {
		t.Fatalf("force http = %v, want false", item.Extra["openai_ws_force_http"])
	}
	if item.Extra["openai_compact_mode"] != "force_on" {
		t.Fatalf("compact mode = %v, want force_on", item.Extra["openai_compact_mode"])
	}
	if item.Extra["openai_long_context_billing_enabled"] != true {
		t.Fatalf("long context billing = %v, want true", item.Extra["openai_long_context_billing_enabled"])
	}
	if item.Extra["codex_cli_only"] != true {
		t.Fatalf("codex cli only = %v, want true", item.Extra["codex_cli_only"])
	}
	if item.Extra["codex_fingerprint_mode"] != "session" {
		t.Fatalf("fingerprint mode = %v, want session", item.Extra["codex_fingerprint_mode"])
	}
	if _, ok := item.Extra["codex_fingerprint_seed"]; ok {
		t.Fatal("codex fingerprint seed must be regenerated by the local account lifecycle")
	}
	if _, ok := item.Extra["access_token"]; ok {
		t.Fatal("credential fields must not be copied into extra")
	}
