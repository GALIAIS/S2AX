...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	// Viper does not reliably coerce a delimited environment value into a
	// string slice. Preserve YAML lists while accepting deployment-friendly
	// SERVER_TRUSTED_PROXIES syntax.
	if trustedProxiesEnvConfigured {
		cfg.Server.TrustedProxies = parseTrustedProxyList(trustedProxiesEnv)
	} else {
