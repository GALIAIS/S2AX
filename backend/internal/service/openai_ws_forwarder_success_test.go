...
func TestOpenAIGatewayService_Forward_WSv2_CodexFingerprintHandshakeBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	wantThread := resolveConvergedChildThreadID(seed, "header-session", "header-thread")
	...
	require.Equal(t, resolveConvergedWindowID(seed, wantThread, 0), captureDialer.lastHeaders.Get("x-codex-window-id"))
	...
	require.Equal(t, resolveConvergedWindowID(seed, wantThread, 0), gjson.Get(payloadJSON, "client_metadata.x-codex-window-id").String())
