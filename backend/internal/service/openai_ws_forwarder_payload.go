...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	// session/full fingerprint 已经覆盖本 attempt 的完整身份；device 只覆盖
	// installation，其他字段仍用 credential namespace 防止跨账号复用。
	...
	applyStagedCodexFingerprintHeadersForTransport(c, account, headers, false)
