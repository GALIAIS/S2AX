...
func TestAccountTestService_TestAccountConnection_OpenAICompactOAuthSuccessPersistsSupport(t *testing.T) {
	...
	require.Equal(t, probeSession, gjson.GetBytes(upstream.lastBody, "prompt_cache_key").String())
	...
	require.NoError(t, err)
	require.Equal(t, uuid.Version(4), parsedInstallationID.Version())
	require.Equal(t, probeSession, gjson.GetBytes(upstream.lastBody, "client_metadata.session_id").String())
	require.Equal(t, probeSession, gjson.GetBytes(upstream.lastBody, "client_metadata.thread_id").String())
	require.False(t, gjson.GetBytes(upstream.lastBody, "client_metadata.window_number").Exists())
	require.False(t, gjson.GetBytes(upstream.lastBody, "client_metadata.context_window_id").Exists())
	turnMetadataRaw := gjson.GetBytes(upstream.lastBody, "client_metadata.x-codex-turn-metadata").String()
	require.Equal(t, "compaction", gjson.Get(turnMetadataRaw, "request_kind").String())
	require.Equal(t, "responses_compaction_v2", gjson.Get(turnMetadataRaw, "compaction.implementation").String())
	require.Equal(t, probeSession+":0", gjson.Get(turnMetadataRaw, "window_id").String())
	require.Equal(t, float64(0), gjson.Get(turnMetadataRaw, "window_number").Float())
	contextWindowID, err := uuid.Parse(gjson.Get(turnMetadataRaw, "context_window_id").String())
	require.NoError(t, err)
	require.Equal(t, uuid.Version(7), contextWindowID.Version())
	require.JSONEq(t, turnMetadataRaw, upstream.lastReq.Header.Get("x-codex-turn-metadata"))
...
func TestAccountTestService_TestAccountConnection_OpenAICompactProbeIdentityMatchesRealTraffic(t *testing.T) {
	...
	require.Empty(t, upstream.lastReq.Header.Get("x-codex-installation-id"),
		"原生 v2 compaction 复用普通 /responses 头部投影")
	...
	require.Equal(t, converged, gjson.GetBytes(upstream.lastBody, "client_metadata.thread_id").String())
...
func TestCompactProbeSessionID_IsUUIDShaped(t *testing.T) {
	for _, id := range []int64{0, 1, 987654} {
		...
		require.Equal(t, uuid.Version(7), parsed.Version(), "探测会话标识必须是 UUIDv7")
	}
