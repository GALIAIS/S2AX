...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(turn int, raw []byte) (openAIWSClientPayload, error) {
		...
		// 直接 WS 与 HTTP 入口共享同一身份优先级：session/full fingerprint
		// 直接接管身份，device 只接管 installation，关闭时才使用 namespace。
		fingerprintIDs := resolveAndStageCodexFingerprintIDsForRaw(c, account, normalized)
		if fingerprintIDs != nil {
			...
			fingerprintPayload, _, fingerprintErr := applyCodexFingerprintClientMetadataRaw(normalized, fingerprintIDs)
		...
		if codexBridgeEnabled {
			...
			// WebSocket Hosted 桥接同样只使用账号配置的图片工具模型，避免不同入口
			// 因隐含默认值产生不同的上游模型。
			if ensureOpenAIResponsesImageGenerationToolWithModel(payloadMap, account.ResolveOpenAIImageToolModel()) {
				bridgeModified = true
