...
// ProvideOpenAIGatewayService 构造 OpenAI 网关并注入 TLS profile 服务。
// 通过 provider 适配保留 NewOpenAIGatewayService 的既有参数契约，避免测试和
// 扩展调用方因新增可选依赖而被迫改造；生产装配仍能使用数据库模板和 Codex 默认模板。
...
// ProvideSettingService wires SettingService with its live-updatable runtime dependencies.
func ProvideSettingService(settingRepo SettingRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, cfg *config.Config, ipGeolocationService *IPGeolocationService) *SettingService {
	svc := NewSettingService(settingRepo, cfg)
	svc.SetDefaultSubscriptionGroupReader(groupRepo)
	svc.SetProxyRepository(proxyRepo)
	svc.SetIPGeolocationService(ipGeolocationService)
	if err := svc.LoadForwardedClientIPSettings(context.Background()); err != nil {
		logger.LegacyPrintf("service.setting", "Warning: load forwarded client IP settings failed: %v", err)
	}
	if err := svc.LoadIPGeolocationSettings(context.Background()); err != nil {
		logger.LegacyPrintf("service.setting", "Warning: load IP geolocation settings failed: %v", err)
	}
...
// ProvideSubscriptionService constructs the subscription service and attaches
// the optional shared-pool admission controller.
func ProvideSubscriptionService(groupRepo GroupRepository, userSubRepo UserSubscriptionRepository, billingCacheService *BillingCacheService, entClient *dbent.Client, cfg *config.Config, sharedQuotaPool *SharedQuotaPoolService) *SubscriptionService {
	svc := NewSubscriptionService(groupRepo, userSubRepo, billingCacheService, entClient, cfg)
	svc.SetSharedQuotaPoolService(sharedQuotaPool)
	return svc
}
