...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	// 独立调用该层时可能没有经过 Forward；补齐一次快照，确保 WS body 与
	// buildOpenAIWSHeaders 使用同一组收敛 ID。正常 HTTP 入口已有 staged snapshot，
	// resolveAndStageCodexFingerprintIDs 在这里会保留其 failover 账号边界。
	if stagedCodexFingerprintIDs(c, account) == nil {
		resolveAndStageCodexFingerprintIDsForMap(c, account, payload)
	}
