...
// ensureOpenAIResponsesImageGenerationToolWithModel 为 Responses 桥接注入账号指定的
// 图片工具模型。未配置模型时保持请求原样，避免偷偷把固定图片模型送到上游。
func ensureOpenAIResponsesImageGenerationToolWithModel(reqBody map[string]any, imageModel string) bool {
	if len(reqBody) == 0 {
		return false
	}
