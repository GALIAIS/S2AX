...
func (m *contentModerationBuiltinRegexMatcher) Match(text string) (contentModerationBuiltinRegexVerdict, bool) {
	...
	for _, rule := range m.rules {
		if !rule.regexp.MatchString(scanText) {
			continue
		}
		...
		if definition.Strict {
			verdict.StrictScore += definition.Weight
		}
	}
	...
	verdict.ContextDiscount = contentModerationBuiltinRegexDefensiveDiscount(scanText)
	verdict.Score = verdict.RawScore - verdict.ContextDiscount
	if verdict.Score < 0 {
		verdict.Score = 0
	}
...
func contentModerationBuiltinRegexReason(verdict contentModerationBuiltinRegexVerdict, threshold, strictThreshold int) string {
	names := make([]string, 0, min(3, len(verdict.Matches)))
	for index, match := range verdict.Matches {
		if index >= 3 {
			break
		}
...
func contentModerationBuiltinRegexMatchedRules(matches []contentModerationBuiltinRegexMatch) string {
	names := make([]string, 0, min(3, len(matches)))
