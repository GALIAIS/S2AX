...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	accountExtra, err = normalizeOpenAIImageGenerationExtra(input.Platform, accountExtra)
	if err != nil {
		return nil, err
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Extra != nil {
		...
		normalizedExtra, err = normalizeOpenAIImageGenerationExtra(account.Platform, normalizedExtra)
		if err != nil {
			return nil, err
		}
...
func (s *adminServiceImpl) UpdateAccountExtra(ctx context.Context, id int64, updates map[string]any) error {
	...
	if _, hasLongContext := updates[openAILongContextBillingEnabledKey]; hasLongContext || hasOpenAIImageGenerationExtra(updates) {
		account, err := s.accountRepo.GetByID(ctx, id)
		if err != nil {
			return err
		}
		...
		if err := ValidateOpenAIImageGenerationExtra(account.Platform, updates); err != nil {
			return err
		}
	}
	if len(updates) == 0 {
