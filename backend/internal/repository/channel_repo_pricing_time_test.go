...
var channelModelPricingTimePricingColumns = []string{
	...
	"input_price_priority", "output_price_priority", "cache_write_price", "cache_write_1h_price", "cache_write_price_priority",
	"cache_read_price", "cache_read_price_priority", "fast_multiplier", "flex_multiplier", "max_reasoning_effort_multiplier", "image_input_price", "image_output_price",
...
func modelPricingTimePricingRow(timePricing any) *sqlmock.Rows {
	return sqlmock.NewRows(channelModelPricingTimePricingColumns).AddRow(
		...
		nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, timePricing,
...
func TestChannelModelPricingTimePricingCreateAndUpdateRoundTrip(t *testing.T) {
	...
	t.Run("create writes JSON", func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta("INSERT INTO channel_model_pricing (channel_id, platform, models, billing_mode, input_price, input_price_priority, output_price, output_price_priority, cache_write_price, cache_write_1h_price, cache_write_price_priority, cache_read_price, cache_read_price_priority, fast_multiplier, flex_multiplier, max_reasoning_effort_multiplier, image_input_price, image_output_price, per_request_price, time_pricing)")).
			WithArgs(
				...
				nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, channelModelPricingTimePricingJSON,
			).
	...
	t.Run("update writes JSON and entry ID", func(t *testing.T) {
		repo, mock := newChannelModelPricingTimePricingRepo(t)
		mock.ExpectExec(`(?s)UPDATE channel_model_pricing.*per_request_price = \$17, time_pricing = \$18, platform = \$19.*WHERE id = \$20`).
			WithArgs(
				[]byte(`["gpt-5"]`), service.BillingModeToken,
				nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, channelModelPricingTimePricingJSON, "openai", int64(11),
			).
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			t.Run("update writes SQL NULL", func(t *testing.T) {
				...
				mock.ExpectExec(`(?s)UPDATE channel_model_pricing.*per_request_price = \$17, time_pricing = \$18, platform = \$19.*WHERE id = \$20`).
					WithArgs(
						[]byte(`["gpt-5"]`), service.BillingModeToken,
						nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "openai", int64(11),
					).
					WillReturnResult(sqlmock.NewResult(0, 1))
