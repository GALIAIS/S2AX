...
--
-- 与 235 同型：DROP IF EXISTS 后重建超集约束，存量行瞬时校验通过。
-- Devin 无 HTTP 额度/监控端点，加入 channel_monitors 仅为保持约束与平台枚举一致。
...
ALTER TABLE user_platform_quotas
    ...
    CHECK (platform IN ('anthropic', 'openai', 'gemini', 'antigravity', 'grok',
                        'kimi', 'zhipu', 'deepseek', 'minimax', 'devin'));
...
BEGIN
    ...
    IF monitor_constraint_def IS NULL OR position('devin' IN monitor_constraint_def) = 0 THEN
        ALTER TABLE channel_monitors
            DROP CONSTRAINT IF EXISTS channel_monitors_provider_check;
        ALTER TABLE channel_monitors
            ADD CONSTRAINT channel_monitors_provider_check
            CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok',
                                'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'devin'));
    END IF;
    ...
    IF template_constraint_def IS NULL OR position('devin' IN template_constraint_def) = 0 THEN
