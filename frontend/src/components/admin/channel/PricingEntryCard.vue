    ...
    >
      <div class="collapsible-inner">
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ t('admin.channels.form.fastPrices') }}
            <span class="ml-1 font-normal text-gray-400">$/MTok · {{ t('admin.channels.form.fastPricesHint') }}</span>
          </label>
          <div class="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <div>
              ...
              <input :value="entry.input_price_priority" @input="emitField('input_price_priority', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.inheritStandardPrice')" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.outputPricePriority') }}</label>
              <input :value="entry.output_price_priority" @input="emitField('output_price_priority', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.inheritStandardPrice')" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWritePricePriorityShort') }}</label>
              <input :value="entry.cache_write_price_priority" @input="emitField('cache_write_price_priority', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.inheritStandardPrice')" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadPricePriorityShort') }}</label>
              <input :value="entry.cache_read_price_priority" @input="emitField('cache_read_price_priority', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.inheritStandardPrice')" />
            </div>
          </div>
...
function addInterval() {
  ...
  intervals.push({
    ...
    cache_read_price: null, input_price_priority: null, output_price_priority: null,
    cache_write_price_priority: null, cache_read_price_priority: null, per_request_price: null,
...
async function onModelsUpdate(newModels: string[]) {
  ...
  const hasPrice = e.input_price != null || e.output_price != null ||
                   ...
                   e.input_price_priority != null || e.output_price_priority != null ||
                   e.cache_write_price_priority != null || e.cache_read_price_priority != null
  ...
  try {
    ...
    if (result.found) {
      emit('update', {
        ...
        input_price_priority: perTokenToMTok(result.input_price_priority ?? null),
        output_price: perTokenToMTok(result.output_price ?? null),
        output_price_priority: perTokenToMTok(result.output_price_priority ?? null),
        cache_write_price: perTokenToMTok(result.cache_write_price ?? null),
        cache_write_price_priority: perTokenToMTok(result.cache_write_price_priority ?? null),
        cache_write_1h_price: perTokenToMTok(result.cache_write_1h_price ?? null),
        cache_read_price: perTokenToMTok(result.cache_read_price ?? null),
        cache_read_price_priority: perTokenToMTok(result.cache_read_price_priority ?? null),
