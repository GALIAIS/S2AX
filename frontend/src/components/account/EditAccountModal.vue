<template>
  ...
  >
    ...
    >
      ...
      <!-- OpenAI 图片桥接模型配置；模型 ID 来自管理员输入，不在前端写死模型名单。 -->
      <div
        v-if="account?.platform === 'openai' && (account?.type === 'oauth' || account?.type === 'setup-token' || account?.type === 'apikey')"
        class="space-y-4 border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div>
          <label class="input-label">{{ t('admin.accounts.openai.imageModelDefault') }}</label>
          <input
            v-model="openAIImageModel"
            type="text"
            class="input font-mono"
            :placeholder="t('admin.accounts.openai.createImageModel')"
            data-testid="edit-openai-image-model"
          />
          <p class="input-hint">{{ t('admin.accounts.openai.imageModelDefaultDesc') }}</p>
        </div>
        <div>
          <label class="input-label">{{ t('admin.accounts.openai.imageModels') }}</label>
          <textarea
            v-model="openAIImageModels"
            rows="2"
            class="input font-mono"
            :placeholder="t('admin.accounts.openai.createImageModel')"
            data-testid="edit-openai-image-models"
          ></textarea>
          <p class="input-hint">{{ t('admin.accounts.openai.imageModelsDesc') }}</p>
        </div>
        <div v-if="account?.type === 'oauth' || account?.type === 'setup-token'">
          <label class="input-label">{{ t('admin.accounts.openai.imageTextModel') }}</label>
          <input
            v-model="openAIImageTextModel"
            type="text"
            class="input font-mono"
            :placeholder="t('admin.accounts.openai.createTextModel')"
            data-testid="edit-openai-image-text-model"
          />
          <p class="input-hint">{{ t('admin.accounts.openai.imageTextModelDesc') }}</p>
        </div>
      </div>
      ...
      <!-- Codex 指纹收敛模式（OpenAI OAuth/SetupToken） -->
      <div
        v-if="account?.platform === 'openai' && (account?.type === 'oauth' || account?.type === 'setup-token')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
...
// 将图片模型目录文本统一转换为去重后的模型 ID 数组，兼容换行、逗号和分号。
const parseOpenAIImageModelList = (value: string): string[] => Array.from(new Set(
  value
    .split(/[\n,;]+/)
    .map((item) => item.trim())
    .filter(Boolean)
...
const openAIImageModel = ref('')
const openAIImageModels = ref('')
const openAIImageTextModel = ref('')
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  openAIImageModels.value = ''
  openAIImageTextModel.value = ''
  editPlanType.value = ''
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    openAIImageModel.value = typeof extra?.openai_image_model === 'string' ? extra.openai_image_model.trim() : ''
    openAIImageTextModel.value = typeof extra?.openai_image_text_model === 'string' ? extra.openai_image_text_model.trim() : ''
    openAIImageModels.value = Array.isArray(extra?.openai_image_models)
      ? extra.openai_image_models.filter((value): value is string => typeof value === 'string').join('\n')
      : ''
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      const imageModel = openAIImageModel.value.trim()
      if (imageModel) newExtra.openai_image_model = imageModel
      else delete newExtra.openai_image_model
      const imageModels = parseOpenAIImageModelList(openAIImageModels.value)
      if (imageModels.length > 0) newExtra.openai_image_models = imageModels
      else delete newExtra.openai_image_models
      if (props.account.type === 'oauth' || props.account.type === 'setup-token') {
        if (openAIImageTextModel.value.trim()) {
          newExtra.openai_image_text_model = openAIImageTextModel.value.trim()
        } else {
          delete newExtra.openai_image_text_model
        }
      }
