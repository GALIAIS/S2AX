export default {
    ...
    channels: {
      ...
      bulkDeleteConfirm: 'Delete the {count} selected channel(s)? This cannot be undone.',
      bulkDeleteSuccess: 'Deleted {count} channel(s)',
      bulkDeletePartial: 'Deleted {success} channel(s); {failed} failed',
      ...
      form: {
        ...
        fastPrices: 'Fast / priority prices (optional)',
        fastPricesHint: 'Leave blank to use standard prices',
        inputPrice: 'Input',
        inputPricePriority: 'Fast input',
        outputPrice: 'Output',
        outputPricePriority: 'Fast output',
        cacheWritePrice: 'Cache Write',
        cacheWritePricePriorityShort: 'Fast cache W',
        cacheReadPrice: 'Cache Read',
        cacheReadPricePriorityShort: 'Fast cache R',
        ...
        inheritStandardPrice: 'Use standard',
    ...
    riskControl: {
      ...
      configConflict: 'This config was changed by another administrator. Refresh it before applying your changes again.',
      logsFailed: 'Failed to load audit records',
      saved: 'Content moderation config saved',
      refresh: 'Refresh',
      config: 'Content Moderation Config',
      configHint: 'Combine local rules with optional remote moderation, then record or block by mode.',
      ...
      modeObserveDesc: 'Requests always pass; local rules run immediately and remote review runs asynchronously, with hits recorded without ban side effects.',
      modeOff: 'Off',
      modeOffDesc: 'Content moderation is disabled and no audit records are written.',
      baseUrl: 'Audit Service Base URL',
      model: 'Model',
      endpointType: 'Remote moderation endpoint',
      endpointModeration: 'Moderations category scores',
      endpointModerationDesc: 'Uses the provider’s broad built-in safety categories; the custom audit prompt is not used.',
      endpointChat: 'Chat Completions AI audit',
      endpointChatDesc: 'Uses a custom system prompt and confidence threshold for this project’s narrow risk policy.',
      auditPrompt: 'AI audit system prompt',
      auditPromptHint: 'Audited content is wrapped as non-executable data; require a JSON-only decision in the prompt.',
      auditPromptRequired: 'A system prompt is required for Chat Completions auditing',
      confidenceThreshold: 'Violation confidence threshold',
      confidenceThresholdInvalid: 'The confidence threshold must be greater than 0 and at most 1',
      contextMessageLimit: 'Context message count',
      contextMessageLimitHint: 'Audit only the latest 1–20 messages to bound latency and cost.',
      policyCompositionHint: 'Content moderation and Prompt Audit are independent pipelines. If both are enabled, either pipeline may block. A “fallback to local” remote failure requires an actually executed local detector; otherwise the gateway returns unavailable instead of silently allowing.',
      apiKey: 'Audit Service API Key',
      apiKeys: 'Audit Service API Keys',
      ...
      apiKeyTestNoInput: 'Enter audit service API Keys to test first',
      apiKeyTestDone: 'Key test completed for {count} keys',
      apiKeyTestFailed: 'Failed to test audit service API Keys',
      apiKeyRequiredForRemote: 'This policy calls remote moderation, so keep or add at least one audit API key',
      detectorRequired: 'Enable at least one local detector or remote moderation before enabling content moderation',
      ...
      auditTestReason: 'Reason: {reason}',
      auditTestSource: 'Source: {source}',
      ...
      proxy: 'Proxy Server',
      proxyHint: 'Send audit requests through the selected proxy (IP Management - Proxy Servers), useful when the audit service restricts egress IPs. Defaults to direct connection.',
      ...
      groupScopeRequired: 'Select all groups or at least one specific group before enabling content moderation',
      ...
      builtinRegexEnabled: 'Enable local regex controls',
      builtinRegexEnabledHint: 'When enabled, local regex runs independently of the keyword strategy; pre-block rejects hits while observe mode only records them.',
      builtinRegexThreshold: 'Regular hit threshold',
      builtinRegexThresholdHint: 'A regular local hit is raised when the accumulated rule weight reaches this value.',
      builtinRegexStrictThreshold: 'Strict-rule threshold',
      builtinRegexStrictThresholdHint: 'Strict rules at this score receive the higher enforcement level.',
      builtinRegexRules: 'Local regex rules',
      builtinRegexRulesHint: '{count} rules are currently saved. Built-in rules are enabled by default and can be edited or removed one by one.',
      builtinRegexRestoreDefaults: 'Restore built-ins',
      builtinRegexAddRule: 'Add rule',
      builtinRegexEmpty: 'No local regex rules',
      builtinRegexEmptyHint: 'Restore the built-ins or add a custom rule.',
      builtinRegexName: 'Name',
      builtinRegexNamePlaceholder: 'Example: credential_theft',
      builtinRegexCategory: 'Category',
      builtinRegexCategoryPlaceholder: 'Example: malicious',
      builtinRegexWeight: 'Weight',
      builtinRegexStrict: 'Strict',
      builtinRegexPattern: 'Regular expression',
      builtinRegexPatternPlaceholder: 'Enter a Go RE2-compatible expression',
      builtinRegexDeleteRule: 'Delete rule',
      builtinRegexDefaultsUnavailable: 'Built-in rules were not returned by the server',
      builtinRegexRuleRequired: 'Name, category, and pattern are required for local regex rule {position}',
      builtinRegexRuleDuplicate: 'Duplicate local regex rule name: {name}',
      builtinRegexRuleWeightInvalid: 'Weight for local regex rule “{name}” must be between 1 and 1000',
      builtinRegexRulePatternInvalid: 'Pattern for local regex rule “{name}” is invalid',
      tabs: {
        basic: 'Basic',
        scope: 'Scope',
        runtime: 'Runtime',
        response: 'Hit Notice',
        riskThresholds: 'Risk Thresholds',
        builtinRegex: 'Local Regex',
        keywords: 'Keyword Block',
        retention: 'Retention',
      },
      blockedKeywords: 'Blocked keywords',
      blockedKeywordsPlaceholder: 'One keyword per line, e.g.:\nbadword1\nbadword2',
      blockedKeywordsDescription: 'Matching is case-insensitive; pre-block rejects hits while observe mode only records them.',
      blockedKeywordsPreBlockHint: 'Pre-block mode is active, so keyword hits reject the request immediately.',
      blockedKeywordsObserveHint: 'Observe mode is active, so keyword hits are recorded while the request remains allowed.',
      blockedKeywordsModeWarning: 'Keyword detection does not run in {mode} mode.',
      ...
      keywordModeKeywordAndApi: 'Local rules + API',
      keywordModeKeywordAndApiDesc: 'Run keywords and the independent local regex first, then sample local misses for remote moderation.',
      keywordModeKeywordOnly: 'Local rules only',
      keywordModeKeywordOnlyDesc: 'Run keywords, local regex, and available hash checks without calling remote moderation.',
      keywordModeKeywordOnlyNotice: 'Local-only strategy: remote moderation is not called.',
      keywordModeApiOnly: 'Keywords off + API',
      keywordModeApiOnlyDesc: 'Skip the keyword list, keep the independent local regex active, and sample requests for remote moderation.',
      keywordModeApiOnlyNotice: 'The keyword list is disabled; local regex still runs before sampled remote moderation.',
      overview: {
        status: 'Status',
        enabled: 'Enabled',
        disabled: 'Disabled',
    ...
    subscriptions: {
      ...
      sharedMemberMissing: 'Not in shared pool',
      sharedResetIn: 'Refreshes in {time}',
      sharedProviderUsage: 'Official pool used {percent}',
