export default {
    accountAllocations: {
      title: '账号分配控制',
      description: '为指定用户保留分组内的上游账号，并按健康状态自动补齐。',
      create: '新建分配策略',
      edit: '编辑分配策略',
      details: '管理分配',
      detailsTitle: '{user} · {group}',
      rowActions: '用户 {email} 的账号分配操作',
      user: '用户',
      group: '分组',
      selectUser: '选择目标用户',
      selectGroup: '选择账号来源分组',
      noUsers: '没有可选择的启用用户',
      userHint: '策略不会静默修改用户授权；缺少专属分组权限或有效订阅时，策略会保留但暂停分配。',
      groupHint: '只会从该分组中选择空闲、健康且未被租约占用的账号。',
      desiredCount: '目标账号数',
      desiredCountHint: '自动补齐开启时，有效分配数低于该值会从空闲账号池补齐；高于或等于时不会自动回收。当前单策略上限：{max}。',
      autoReplenish: '自动补齐',
      autoReplenishHint: '后台会按策略周期检查容量短缺，不会阻塞用户请求。',
      autoReplenishOn: '自动补齐',
      autoReplenishOff: '仅手动',
      replace401: '替换 401 认证失败账号',
      replace401Hint: '账号出现已持久化的 401 认证失败时释放；已删除、解绑、禁用、到期等硬不可用状态始终释放。',
      replace429: '替换限流冷却账号',
      replace429Hint: '账号处于持久化的 429 冷却窗口时释放，并尝试从空闲池补齐。',
      capacity: '容量',
      activeAssignments: '有效分配',
      shortage: '缺口',
      shortageCount: '缺 {count}',
      capacityMet: '已满足',
      lastReconciled: '最近协调',
      reconcileNow: '立即协调',
      reconcileAll: '协调全部',
      reconciled: '协调完成：补入 {assigned}，释放 {released}，缺口 {shortage}',
      reconciledAll: '已协调 {processed} 个策略：补入 {assigned}，释放 {released}，总缺口 {shortage}',
      reconcileFailed: '账号协调失败',
      reconcileAllFailed: '批量协调账号分配失败',
      manualAssignment: '手动分配',
      manualAssignmentHint: '可从当前分组剩余的健康空闲账号中选择。分配后同一账号会在全局独占。',
      selectCandidate: '选择空闲账号',
      noCandidates: '暂无符合条件的空闲账号',
      policyDisabled: '策略已停用',
      assignManually: '手动分配',
      assigned: '账号已分配',
      assignFailed: '手动分配账号失败',
      assignments: '分配记录',
      noAssignments: '当前没有分配记录',
      account: '账号',
      removedAccount: '已删除账号',
      removedAccountHint: '仅保留分配与审计历史，原账号信息已清除。',
      assignedAt: '分配时间',
      releaseReason: '释放原因',
      assignmentActive: '有效',
      assignmentReleased: '已释放',
      release: '释放分配',
      releaseConfirm: '释放后该账号会重新成为共享池候选；如果自动补齐已开启，系统会优先尝试其他空闲账号，不会在本次操作中把刚释放的账号立即分回。',
      released: '分配已释放',
      releaseFailed: '释放分配失败',
      eventHistory: '事件历史',
      noEvents: '暂无分配事件',
      systemActor: '系统',
      disable: '停用策略',
      enable: '启用策略',
      disableConfirm: '停用会立即释放该策略下的所有有效分配，并停止网关强制路由和自动补齐。是否继续？',
      enableConfirm: '启用后会重新实施租约路由；如果自动补齐开启，系统将按目标数量补齐。是否继续？',
      enabled: '策略已启用',
      disabled: '策略已停用',
      statusFailed: '更新策略状态失败',
      deletePolicy: '删除分配策略',
      deleteConfirm: '删除会释放该策略下的所有有效分配，并保留历史租约和审计事件。删除后可为同一用户和分组重新创建策略。是否继续？',
      deleted: '分配策略已删除',
      deleteFailed: '删除分配策略失败',
      created: '分配策略已创建',
      createFailed: '创建分配策略失败',
      updated: '分配策略已更新',
      updateFailed: '更新分配策略失败',
      loadFailed: '加载分配策略失败',
      detailsLoadFailed: '加载分配详情失败',
      overviewLoadFailed: '加载账号分配概览失败',
      referencesLoadFailed: '加载用户或分组列表失败',
      formRequired: '请选择用户和分组。',
      accessBlockedHint: '系统不会向当前用户继续租用该分组账号。恢复用户状态、分组状态、专属分组权限或订阅后，后台协调器会自动重新补齐。',
      accessStatus: {
        ready: '访问条件正常',
        user_unavailable: '目标用户不可用',
        group_unavailable: '来源分组不可用',
        group_access_required: '缺少专属分组权限',
        subscription_required: '缺少有效订阅'
      },
      usageVisibility: {
        manage: '用量可见权限',
        title: '账号目录用量可见权限',
        description: '向账号目录下发已缓存的 5 小时 / 7 天配额窗口及授权分组内的窗口聚合，不会主动查询上游，也不会授予分组、API Key 或调度权限。',
        readOnlyNotice: '只读明细授权',
        scope: '授权范围',
        exclusiveGroup: '专属分组用户',
        exclusiveGroupHint: '该专属分组中当前及后续具备权限或有效订阅的用户，均可查看组内账号的缓存配额与分组内窗口用量。',
        userAccount: '指定用户与账号',
        userAccountHint: '仅指定用户可查看指定分组内单个账号的缓存配额与分组内窗口用量。',
        selectAccount: '选择账号',
        selectGroupFirst: '请先选择分组',
        noExclusiveGroups: '暂无启用中的专属分组',
        noAccounts: '该分组暂无匹配账号',
        activeGrants: '当前授权',
        empty: '尚未配置额外的用量明细授权。',
        revokeHint: '撤销后用户下次刷新账号目录即失去明细可见性；专属分配本身提供的明细权限不受影响。',
        target: '授权对象',
        allEligibleUsers: '该专属分组的全部合资格用户',
        createdAt: '创建时间',
        create: '创建授权',
        revoke: '撤销授权',
        revokeConfirm: '确定撤销该用量明细授权吗？用户下次刷新账号目录后将不再看到对应的缓存配额。',
        groupRequired: '请选择授权分组。',
        userAccountRequired: '请选择用户和账号。',
        created: '用量明细授权已创建',
        createFailed: '创建用量明细授权失败',
        revoked: '用量明细授权已撤销',
        revokeFailed: '撤销用量明细授权失败',
        loadFailed: '加载用量明细授权失败',
        accountsLoadFailed: '加载分组账号失败'
      },
      overview: {
        activePolicies: '启用策略',
        allocatedCapacity: '已分配 / 目标',
        shortage: '账号缺口',
        policyUnit: '{count} 个策略',
        blockedPolicies: '访问受阻策略'
      },
      filters: {
        status: '策略状态',
        user: '用户',
