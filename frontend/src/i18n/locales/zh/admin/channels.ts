export default {
    ...
    channels: {
      ...
      updateError: '更新渠道失败',
      deleteError: '删除渠道失败',
      selectedCount: '已选择 {count} 个渠道',
      bulkDelete: '批量删除',
      bulkDeleteConfirm: '确定要删除选中的 {count} 个渠道吗？此操作不可撤销。',
      bulkDeleteSuccess: '已删除 {count} 个渠道',
      bulkDeletePartial: '已删除 {success} 个渠道，{failed} 个删除失败',
      ...
      form: {
        ...
        fastPrices: 'Fast / priority 价格（可选）',
        fastPricesHint: '留空沿用标准价格',
        inputPrice: '输入',
        inputPricePriority: 'Fast 输入',
        outputPrice: '输出',
        outputPricePriority: 'Fast 输出',
        cacheWritePrice: '缓存写入',
        cacheWritePricePriorityShort: 'Fast 缓存写',
        cacheWrite5mPrice: '缓存写入（5m）',
        cacheWrite1hPrice: '缓存写入（1h）',
        cacheReadPrice: '缓存读取',
        cacheReadPricePriorityShort: 'Fast 缓存读',
    ...
    riskControl: {
      ...
      configConflict: '配置已被其他管理员更新，请刷新配置后重新应用本次修改。',
      logsFailed: '加载审核记录失败',
      saved: '内容审计配置已保存',
      refresh: '刷新',
      config: '内容审计配置',
      configHint: '组合本地规则与可选远程审核，按模式记录或拦截请求。',
      openSettings: '内容审计设置',
      ...
      modeObserve: '仅观察',
      modeObserveDesc: '请求始终放行；本地规则即时检测，远程审核异步执行，命中仅记录且不触发封禁副作用。',
      modeOff: '关闭',
      modeOffDesc: '不执行内容审计，也不会写入审核记录。',
      baseUrl: '审核服务 Base URL',
      model: '模型名',
      endpointType: '远程审核接口',
      endpointModeration: 'Moderations 分类评分',
      endpointModerationDesc: '使用服务商内置的广义安全分类；自定义审核提示词不会参与判定。',
      endpointChat: 'Chat Completions AI 审核',
      endpointChatDesc: '使用自定义系统提示词和置信度阈值，可限定为当前项目的窄域风险策略。',
      auditPrompt: 'AI 审核系统提示词',
      auditPromptHint: '待审核内容会以不可执行的数据封装后发送；提示词必须要求只返回 JSON 判定。',
      auditPromptRequired: 'Chat Completions 审核必须填写系统提示词',
      confidenceThreshold: '违规置信度阈值',
      confidenceThresholdInvalid: '违规置信度阈值必须大于 0 且不超过 1',
      contextMessageLimit: '上下文消息数',
      contextMessageLimitHint: '仅提取最近 1–20 条消息参与审核，避免无界上下文增加延迟和成本。',
      policyCompositionHint: '内容审计与“提示词审计”是两条独立链路；两者同时启用时任一阻断结果都会拒绝请求。远程失败选择“回退本地”时，必须有实际运行的本地规则，否则返回服务不可用而不会静默放行。',
      apiKey: '审核服务 API Key',
      apiKeys: '审核服务 API Keys',
      ...
      testingApiKeys: '测试中',
      apiKeyTestNoInput: '请先输入需要测试的审核服务 API Key',
      apiKeyTestDone: 'Key 测试完成，共 {count} 个',
      apiKeyTestFailed: '测试审核服务 API Key 失败',
      apiKeyRequiredForRemote: '当前策略会调用远程审核接口，至少需要保留或新增 1 个审核 API Key',
      detectorRequired: '启用内容审计时至少需要启用一种本地规则或远程审核',
      ...
      auditTestReason: '判定理由：{reason}',
      auditTestSource: '判定来源：{source}',
      ...
      sampleRate: '采样率',
      proxy: '代理服务器',
      proxyHint: '审计请求经指定代理（IP 管理-代理服务器）发出，适用于审核服务限制出口 IP 的部署；默认直连。',
      ...
      groupScopeRequired: '启用内容审计时必须选择全部分组或至少 1 个指定分组',
      selectedGroups: '指定分组',
      ...
      builtinRegexEnabled: '启用本地正则风控',
      builtinRegexEnabledHint: '启用后，本地正则独立于关键词策略先执行；前置拦截模式会拒绝命中，观察模式只记录命中。',
      builtinRegexThreshold: '普通命中阈值',
      builtinRegexThresholdHint: '累计规则权重达到该值时判定为普通本地命中。',
      builtinRegexStrictThreshold: '严格规则阈值',
      builtinRegexStrictThresholdHint: '严格规则达到该值时直接提高处置等级。',
      builtinRegexRules: '本地正则规则',
      builtinRegexRulesHint: '当前保存 {count} 条规则。内置规则默认启用，也可以逐条修改或删除。',
      builtinRegexRestoreDefaults: '恢复内置规则',
      builtinRegexAddRule: '新增规则',
      builtinRegexEmpty: '暂无本地正则规则',
      builtinRegexEmptyHint: '可以恢复默认规则，或新增一条自定义规则。',
      builtinRegexName: '名称',
      builtinRegexNamePlaceholder: '例如：credential_theft',
      builtinRegexCategory: '分类',
      builtinRegexCategoryPlaceholder: '例如：malicious',
      builtinRegexWeight: '权重',
      builtinRegexStrict: '严格',
      builtinRegexPattern: '正则表达式',
      builtinRegexPatternPlaceholder: '输入兼容 Go RE2 的正则表达式',
      builtinRegexDeleteRule: '删除规则',
      builtinRegexDefaultsUnavailable: '服务端尚未返回内置规则，无法恢复默认值',
      builtinRegexRuleRequired: '第 {position} 条本地正则的名称、分类和表达式不能为空',
      builtinRegexRuleDuplicate: '本地正则规则名称重复：{name}',
      builtinRegexRuleWeightInvalid: '本地正则规则「{name}」的权重必须在 1 到 1000 之间',
      builtinRegexRulePatternInvalid: '本地正则规则「{name}」的表达式无效',
      tabs: {
        basic: '基础',
        scope: '审计范围',
        runtime: '运行队列',
      ...
      blockedKeywordsDescription: '匹配忽略大小写；前置拦截模式会拒绝命中，观察模式只记录命中。',
      blockedKeywordsPreBlockHint: '当前为前置拦截模式，关键词命中会立即拒绝请求。',
      blockedKeywordsObserveHint: '当前为观察模式，关键词命中会被记录但请求仍会放行。',
      blockedKeywordsModeWarning: '当前为「{mode}」模式，关键词检测不会运行。',
      blockedKeywordCount: '已配置 {count} 个关键词',
      blockedKeywordsLimit: '最多保存 {max} 个关键词，单个长度不超过 200 个字符；重复项会自动去重。',
      keywordBlockingMode: '审计策略',
      keywordModeKeywordAndApi: '本地规则 + API',
      keywordModeKeywordAndApiDesc: '先执行关键词和独立本地正则；本地未命中时再按采样率调用远程审核。',
      keywordModeKeywordOnly: '仅本地规则',
      keywordModeKeywordOnlyDesc: '执行关键词、本地正则及可用的哈希检测，不调用远程审核接口。',
      keywordModeKeywordOnlyNotice: '当前为「仅本地规则」策略：不会调用远程审核接口。',
      keywordModeApiOnly: '关闭关键词 + API',
      keywordModeApiOnlyDesc: '不执行关键词列表，仍会执行独立本地正则，并按采样率调用远程审核。',
      keywordModeApiOnlyNotice: '当前策略关闭关键词列表；本地正则仍会运行，请求随后按采样率交给远程审核。',
      overview: {
        status: '运行状态',
        enabled: '已启用',
        disabled: '未启用',
    ...
    subscriptions: {
      ...
      officialPercentFallback: '官方百分比分摊（估算）',
      ...
      sharedProviderUsage: '官方池已用 {percent}',
      noLimits: '未配置限额',
      unlimited: '无限制',
      resetNow: '即将重置',
