...
// ── Platform config ──
const platformOrder: GroupPlatform[] = ['anthropic', 'openai', 'gemini', 'antigravity', 'grok', 'kimi', 'zhipu', 'deepseek', 'minimax', 'devin']
// Composite pricing/mapping may target every concrete schedulable provider.
const compositePlatforms: GroupPlatform[] = ['anthropic', 'openai', 'gemini', 'antigravity', 'grok', 'kimi', 'zhipu', 'deepseek', 'minimax', 'devin']
      ...
      rules.push({
        ...
        pricing: rule.pricing
          ...
          .map(p => ({
            ...
            input_price_priority: mTokToPerToken(p.input_price_priority),
            output_price: mTokToPerToken(p.output_price),
            output_price_priority: mTokToPerToken(p.output_price_priority),
            cache_write_price: mTokToPerToken(p.cache_write_price),
            cache_write_price_priority: mTokToPerToken(p.cache_write_price_priority),
            cache_write_1h_price: mTokToPerToken(p.cache_write_1h_price),
            cache_read_price: mTokToPerToken(p.cache_read_price),
            cache_read_price_priority: mTokToPerToken(p.cache_read_price_priority),
  ...
  for (const section of form.platforms) {
    ...
    for (const entry of section.model_pricing) {
      ...
      model_pricing.push({
        ...
        input_price_priority: mTokToPerToken(entry.input_price_priority),
        output_price: mTokToPerToken(entry.output_price),
        output_price_priority: mTokToPerToken(entry.output_price_priority),
        cache_write_price: mTokToPerToken(entry.cache_write_price),
        cache_write_price_priority: mTokToPerToken(entry.cache_write_price_priority),
        cache_read_price: mTokToPerToken(entry.cache_read_price),
        cache_read_price_priority: mTokToPerToken(entry.cache_read_price_priority),
  ...
  for (const platform of platformOrder) {
    ...
    const pricing = (channel.model_pricing || [])
      ...
      .map(p => ({
        ...
        input_price_priority: perTokenToMTok(p.input_price_priority),
        output_price: perTokenToMTok(p.output_price),
        output_price_priority: perTokenToMTok(p.output_price_priority),
        cache_write_price: perTokenToMTok(p.cache_write_price),
        cache_write_price_priority: perTokenToMTok(p.cache_write_price_priority),
        cache_read_price: perTokenToMTok(p.cache_read_price),
        cache_read_price_priority: perTokenToMTok(p.cache_read_price_priority),
